...
router.get("/wb-live/products", async (req, res) => {
  ...
  const allOrders = await wbGet<any[]>(`/api/v1/supplier/orders?dateFrom=${from}&flag=0`);
  await sleep(500);
  const allSales = await wbGet<any[]>(`/api/v1/supplier/sales?dateFrom=${from}&flag=0`);
...
router.get("/wb-live/stocks", async (_req, res) => {
  ...
  const allStocks = await wbGet<any[]>(`/api/v1/supplier/stocks?dateFrom=${yesterday}`);
